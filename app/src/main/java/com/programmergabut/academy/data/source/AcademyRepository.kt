package com.programmergabut.academy.data.source
...
import com.programmergabut.academy.data.source.remote.response.ModuleResponse
...
class AcademyRepository private constructor(private val remoteDataSource: RemoteDataSource) : AcademyDataSource {
    ...
    override fun getAllCourses(): MutableLiveData<List<CourseEntity>> {
        ...
        remoteDataSource.getAllCourses(object : RemoteDataSource.LoadCoursesCallback {
            override fun onAllCoursesReceived(courseResponses: List<CourseResponse>) {
                ...
                for (response in courseResponses) {
                    ...
                    val course = CourseEntity(response.id, response.title, response.description,
    ...
    override fun getBookmarkedCourses(): MutableLiveData<List<CourseEntity>> {
        val courseResults = MutableLiveData<List<CourseEntity>>()
            ...
            override fun onAllCoursesReceived(courseResponses: List<CourseResponse>) {
                ...
                for (response in courseResponses) {
                    ...
                    if (response.id == courseId)
                        course = CourseEntity(response.id, response.title, response.description,
                            response.date, false, response.imagePath)
    ...
    override fun getAllModulesByCourse(courseId: String): MutableLiveData<List<ModuleEntity>> {
        ...
        remoteDataSource.getModules(courseId, object : RemoteDataSource.LoadModulesCallback {
            override fun onAllModulesReceived(moduleResponses: List<ModuleResponse>) {
                ...
                for(response in moduleResponses) {
                    ...
                    val course = ModuleEntity(response.moduleId, response.courseId,
                        response.title, response.position, false)
